 ...
 * policy, the best action is chosen with probability <b>1-epsilon</b>.
 * Otherwise, with probability <b>epsilon</b>, any other action, except the best
 * one, is chosen randomly.</para>
 * 
...
public class EpsilonGreedyExploration implements IExplorationPolicy {
	...
	/**
	 * The value determines the amount of exploration driven by the policy. If the
	 * value is high, then the policy drives more to exploration - choosing random
	 * action, which excludes the best one. If the value is low, then the policy is
	 * more greedy - choosing the beat so far action.
	 * 
	...
	public int ChooseAction(double[] actionEstimates, boolean[] actionRestriction) {
		...
		for (int i = 1; i < actionsCount; i++) {
			if (actionEstimates[i] > maxReward && actionRestriction[i] == true) {
				maxReward = actionEstimates[i];
				greedyAction = i;
				totalActionValue = totalActionValue + actionEstimates[i];
			}
		}

		// try to do exploration
		if (r.nextDouble() < epsilon || totalActionValue == 0) {
			int randomAction;
			while (true) {
				randomAction = r.nextInt(actionsCount);
				if (actionRestriction[randomAction] == true) {
					return randomAction;
				}
